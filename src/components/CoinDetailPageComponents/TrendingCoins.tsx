...
const TrendingCoins = () => {
  ...
  return (
    <div className={"relative top-6 sm:mr-4"}>
      {" "}
      {/* agar mobile pe center nahi hora toh remove this margin right  */}
      <p className="text-2xl font-medium lg:left-36 md:left-24 mb-8 relative -top-2 interFont ml-6">
        Market Trend{" "}
      </p>
          ...
          >
            {topCoins.map((item, index) => {
              return (
                <div
                  key={item.id + index}
                  className="boxshDark w-80 h-44 rounded-md flex flex-col"
                >
                  <div className="trendingUpperHold justify-between items-center flex gap-4 pl-4 mt-4">
                    <Link href={`/${item.id}`}>
                      <div className="trendingUpper-1 flex items-center gap-2">
                        ...
                        <p className="bg-gray-100 whitespace-nowrap capitalize rounded font-normal w-auto pr-2 pl-2 h-7">
                          {" "}
                  ...
                  <div className="flex items-center text-sm justify-between gap-4 pr-0 pl-0">
                    <div className="flex text-sm items-center gap-1 pl-3 pr-2">
                      <p className="font-semibold whitespace-nowrap text-xs text-black ">
                        24H High:
                      </p>
                      <p className="whitespace-nowrap  text-green-500 text-sm flex items-center gap-1">
                        {" "}
                        {item.high_24h.toLocaleString()}{" "}
                        <FiTrendingUp className="text-green-500 font-semibold text-lg" />{" "}
                      </p>
                    </div>
                    ...
                    <div className="flex text-sm items-center gap-1 pl-3 pr-2">
                      ...
                      <p className="whitespace-nowrap  text-red-500 text-sm flex items-center gap-1">
                        {item.low_24h.toLocaleString()}{" "}
                        <FiTrendingDown className="text-red-500 font-semibold text-lg" />{" "}
                      </p>
                    </div>
                  </div>
                </div>
