    ...
    const fetchDataHandler = async () => {
        ...
        try {
            const response1 = await fetch(
                `https://api.coingecko.com/api/v3/coins/${props.cryptoName}?tickers=false&community_data=false&developer_data=false`, options
            );
            if (!response1.ok) {
        ...
        try {
            const response2 = await fetch('https://api.coingecko.com/api/v3/search/trending',options);
            if (!response2.ok) {
